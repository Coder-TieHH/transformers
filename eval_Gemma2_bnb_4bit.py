...
ADD_CSV = '/root/NEWDATA/ds_add_train.csv'
TRAIN_CSV = "/root/NEWDATA/ds_train_new.csv" # 训练集路径
VAL_CSV = "/root/NEWDATA/ds_val_new.csv" 
model_path = "/root/google/gemma-2-9b-it-bnb-4bit"# 模型路径
...
base_model_0 = Gemma2ForSFT.from_pretrained(
    model_path,
    num_labels=3,
    use_cache=False,
    device_map='cuda')
model = PeftModel.from_pretrained(base_model_0, model_id=WEIGHTS_PATH).to(device0)
model.eval()
# torch.save(model.lm_head.weight 'lm_head_weights.pth')
# for k, v in model_0.parameters():
#     print(k)

args = TrainingArguments(
    output_dir="/root/output/Gemma2_bnb_4bit",
    # all_data
    logging_steps=1000,
    save_steps=2000,
...
# plt.ylabel('Loss')
# plt.grid(True)  # 显示网格  
# plt.savefig('./lossPNG/Gemma2_bnb_4bit.png')
# # 显示图表  
# plt.show()

# # 保存模型
# trainer.model.save_pretrained("/root/result/Gemma2_bnb_4bit_lmhead")
# tokenizer.save_pretrained("/root/result/Gemma2_bnb_4bit_lmhead")